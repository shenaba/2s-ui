...
2S-UI — открытая панель управления [sing-box](https://github.com/SagerNet/sing-box), рассчитанная на развёртывание и эксплуатацию собственных прокси-сервисов. Настройка протоколов, правила маршрутизации, пользователи и подписки, статистика трафика собраны в одном интерфейсе — на шести языках и в двух темах; работает как на одной машине, так и в составе кластера.

2S-UI вырос из форка [s-ui](https://github.com/alireza0/s-ui): фронтенд переписан целиком, а сверху добавлены кластер из нескольких узлов, автоматический выпуск и продление сертификатов ACME, обновление прямо из панели и обновление пользователей на лету, не разрывающее уже установленные соединения.
...
- **Мультипротокольность** — VLESS, VMess, Trojan, Shadowsocks, Hysteria2, TUIC,
  AnyTLS и другие, на входящих и исходящих, плюс endpoints WireGuard/WARP/Tailscale
  ([полный список](#protocols))
- **TLS в одном месте** — Reality, отпечатки uTLS, XTLS; сертификат регистрируется
  один раз, а затем выбирается для каждого входящего
- **Правила маршрутизации в интерфейсе** — совпадение по домену, IP, порту,
  протоколу, процессу, пользователю или rule-set, с объединением через and/or.
  У DNS свой отдельный список правил.
- **Клиент на нескольких входящих** — один клиент живёт сразу на многих входящих,
  со своим лимитом трафика и сроком действия; при превышении любого из них клиент
  отключается автоматически
- **Автоматика по квотам** — отсчёт может начинаться с первого использования и
  сбрасываться каждые N дней, сам возвращая в строй исчерпавшего лимит клиента
- **Лимит IP на клиента** — сколько исходных адресов клиент может занимать
  одновременно; лишние отключаются и ненадолго не пускаются, fail2ban не нужен
- **Обновление пользователей на лету** — правка клиента переписывает таблицу
  пользователей входящего на месте, не пересоздавая слушателя, поэтому остальные
  не теряют соединение
...
- **Кластер из нескольких узлов** — мониторинг других панелей 2S-UI, общие
...
- **Живая панель мониторинга** — ресурсы системы, трафик, распределение по
  протоколам, пропускная способность сети, состояние узлов; каждую плитку можно
  отключить
- **Доступ и локализация** — несколько администраторов панели,
  [API-токены](https://github.com/shenaba/2s-ui/wiki/API-Documentation) со сроком
  действия, тёмная/светлая тема, шесть языков
...
Другие скриншоты интерфейса: [frontend/screenshots.md](frontend/screenshots.md)

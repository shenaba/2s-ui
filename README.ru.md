...
### Обновление прямо из панели

После установки новые релизы отмечаются на плашке версии в боковой панели —
проверку выполняет браузер, поэтому самому серверу с панелью доступ к GitHub не
нужен. На Linux (systemd или Docker) обновление выполняется в одно нажатие:
панель скачивает релиз, сверяет его с опубликованным `SHA256SUMS`, пробно
запускает новый бинарник, затем заменяет его и перезапускается. Без SSH.

...
export default {
  ...
  setting: {
    ...
    webUriPathMismatch: "Path differs from Base URI. This only sets the redirect target — to move the panel, edit Base URI.",
    webUriPathBlocked: "Panel URI path does not match Base URI",
    webUriPathBlockedHint: "Nothing was saved. Panel URI only decides where you are redirected after a restart — it does not change the path the panel serves, so a mismatch drops you on a 404 with no way back. Make them match: edit Base URI to move the panel, or set Panel URI's path back to {path}.",
    ...
    subUriPathMismatch: "Path differs from Default Path. This only sets the link prefix sent to clients — to move it, edit Default Path.",
    subUriPathBlocked: "Subscription URI path does not match Default Path",
    subUriPathBlockedHint: "Nothing was saved. Subscription URI only sets the prefix of the links handed to clients — it does not change the path the subscription serves. A mismatch does not fail loudly: the panel keeps working and every client silently stops updating. Make them match: edit Default Path to move it, or set Subscription URI's path back to {path}.",

...
export default {
  ...
  setting: {
    ...
    webUriPathMismatch: "路径与「面板路径」不一致。这一项只决定跳转到哪;要换路径请改「面板路径」。",
    webUriPathBlocked: "面板 URI 的路径与面板路径不一致",
    webUriPathBlockedHint: "未保存。「面板 URI」只决定重启后跳到哪,不会改变面板实际服务的路径——不一致就会在重启后把你送到一个 404,而那时已经没有回头路。请把两者改成一致:要换路径就改「面板路径」,要保留现在的路径就把「面板 URI」的路径改回 {path}。",
    sessionAge: "会话超时时限",
    trafficAge: "流量过期时限",
    timeLoc: "时区",
    ...
    update: "自动更新时间",
    subUri: "订阅 URI",
    subUriPathMismatch: "路径与「默认路径」不一致。这一项只决定发给客户端的链接前缀;要换请改「默认路径」。",
    subUriPathBlocked: "订阅 URI 的路径与默认路径不一致",
    subUriPathBlockedHint: "未保存。「订阅 URI」只决定发给客户端的链接前缀,不会改变订阅实际服务的路径。不一致不会当场报错——面板一切正常,只是所有客户端从此静默地更新不到。请把两者改成一致:要换路径就改「默认路径」,要保留现在的路径就把「订阅 URI」的路径改回 {path}。",
    jsonSub: "JSON 订阅",
    toDirect: "路由到直连",
    toBlock: "路由到阻止",
    timestamp: "时间戳",

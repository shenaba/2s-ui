...
export default {
  ...
  setting: {
    ...
    webUriPathMismatch: "Путь отличается от «Базового URI». Это только адрес перехода — чтобы перенести панель, измените «Базовый URI».",
    webUriPathBlocked: "Путь «URI панели» не совпадает с «Базовым URI»",
    webUriPathBlockedHint: "Ничего не сохранено. «URI панели» задаёт только адрес перехода после перезапуска и не меняет путь, который обслуживает панель, — при расхождении вы попадёте на 404 без пути назад. Приведите их в соответствие: измените «Базовый URI», чтобы перенести панель, или верните путь «URI панели» на {path}.",
    ...
    subUri: "URI подписки",
    subUriPathMismatch: "Путь отличается от «Пути по умолчанию». Это только основа ссылок для клиентов — чтобы перенести подписку, измените «Путь по умолчанию».",
    subUriPathBlocked: "Путь «URI подписки» не совпадает с «Путём по умолчанию»",
    subUriPathBlockedHint: "Ничего не сохранено. «URI подписки» задаёт только основу ссылок, выдаваемых клиентам, и не меняет путь, который обслуживает подписка. Расхождение не даёт явной ошибки: панель работает как обычно, а все клиенты молча перестают обновляться. Приведите их в соответствие: измените «Путь по умолчанию», чтобы перенести подписку, или верните путь «URI подписки» на {path}.",

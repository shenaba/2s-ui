...
export default {
  ...
  setting: {
    ...
    webUriPathMismatch: "Его путь отличается от «Базового URI». Это поле задаёт только адрес перехода и не меняет путь, который панель обслуживает на самом деле, — чтобы перенести панель, измените «Базовый URI».",
    ...
    subUri: "URI подписки",
    subUriPathMismatch: "Его путь отличается от «Пути по умолчанию». Это поле задаёт только основу ссылок, выдаваемых клиентам, и не меняет путь, который обслуживает подписка, — чтобы перенести её, измените «Путь по умолчанию».",

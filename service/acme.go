...
func buildVhostConf(opt VhostOptions, certFile, keyFile string, env nginxEnv) string {
	...
	for _, ep := range opt.Endpoints {
		...
		locations.WriteString("    location " + path + " {\n" +
			...
			"        # 且剥掉逐跳头,少了这三行 WebSocket 握手必 400。用 $http_connection 原样\n" +
			"        # 透传而不是写死 \"upgrade\":后者会让每个普通请求也带上 Connection: upgrade,\n" +
			"        # 而本文件里订阅服务那个 location 根本不说 WebSocket。常见的\n" +
			"        # map $http_upgrade 写法在这里用不了——map 只能放 http{} 级,而本文件按\n" +
			"        # 域名各生成一份,重复定义变量会让 nginx 拒载。\n" +

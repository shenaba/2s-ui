...
func panelLocation() *time.Location {
	...
	loc := resolvePanelLocation(name)
	panelLoc.name, panelLoc.loc = name, loc
	return loc
}

func resolvePanelLocation(name string) *time.Location {
...
// Re-anchor only boundaries generated by the previous monthly schedule.
// A manually chosen NextReset is an explicit override for this period, and an
// already-due boundary must still fire rather than being postponed by a zone
// change. The settings write and these updates share one transaction.
func rebaseMonthlyResets(tx *gorm.DB, oldZone, newZone string, now int64) error {
	oldLoc, newLoc := resolvePanelLocation(oldZone), resolvePanelLocation(newZone)
	var clients []model.Client
	if err := tx.Model(model.Client{}).
		Select("id", "reset_day_of_month", "next_reset").
		Where("auto_reset = ? AND delay_start = ? AND reset_day_of_month > 0 AND next_reset > ?", true, false, now).
		Find(&clients).Error; err != nil {
		return err
	}
	for _, client := range clients {
		oldBoundary := nextMonthlyReset(time.Unix(now, 0).In(oldLoc), client.ResetDayOfMonth).Unix()
		if client.NextReset != oldBoundary {
			continue
		}
		// Map the old boundary's calendar month to the new zone. Asking for
		// the next boundary from now could skip this month when its new-zone
		// midnight has passed, or reset twice after the old-zone boundary ran.
		oldDate := time.Unix(oldBoundary, 0).In(oldLoc)
		day := min(client.ResetDayOfMonth, daysInMonth(oldDate.Year(), oldDate.Month()))
		newBoundary := time.Date(oldDate.Year(), oldDate.Month(), day, 0, 0, 0, 0, newLoc).Unix()
		if newBoundary <= now {
			newBoundary = now - 1 // let the next reset tick finish this period
		}
		if newBoundary != oldBoundary {
			if err := tx.Model(model.Client{}).Where("id = ?", client.Id).
				UpdateColumn("next_reset", newBoundary).Error; err != nil {
				return err
			}
		}
	}
